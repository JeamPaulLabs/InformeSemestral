#!/usr/bin/env node
/**
 * PptxGenJS v6: HYBRID — screenshot WITH text + editable overlays
 * Simple leaf-text extraction, SVG text included, icon overlap avoided
 */
...
async function main(){
    ...
    try{
      ...
      for(const variant of variants){
        ...
        const textData=await page.evaluate(()=>{
          ...
          // 1) Table cells: full cell as one block, valign:'middle' to match browser default
          active.querySelectorAll('td,th').forEach(cell=>{
            ...
            // Detect if cell is bold (th, or has strong children, or bold font-weight)
            const cellBold=style.fontWeight==='bold'||parseInt(style.fontWeight)>=600||
          ...
          // 2) Regular text elements (skip children of td/th — already extracted as cell blocks)
          active.querySelectorAll('*').forEach(el=>{
            ...
            // Skip elements that CONTAIN icon children (title text overlaps with icon visually)
            if(el.querySelector('.lucide-ico,.ico')) return;
        ...
        for(const t of filtered){
          ...
          // CSS padding px → PPT points (*72/96 = *0.75). valToPts() expects points.
          const margin=[t.padT*0.75,t.padR*0.75,t.padB*0.75,t.padL*0.75];

          // Compensación visual: -1.5 CSS px (0.0156") en Y para celdas (Raleway PPT vs browser)
  ...
  const outPath=path.join(DIR,'INFORME_TELEVENTAS_HIBRIDO_V2.pptx');
