#!/usr/bin/env node
/**
 * PptxGenJS v3: Skeleton + editable overlays
 * Key fix: extract <td>/<th> as whole blocks, include SVG text, detect icon overlaps
 */
    ...
    try {
      ...
      for (const variant of variants) {
        ...
        const textData = await page.evaluate(() => {
          ...
          // Icon positions for overlap detection
          const iconRects = [];
          ...
          // Collect ALL text, giving priority to <td>/<th> cells (full cell blocks)
          // Then fall back to leaf text nodes
          const cellTexts = [];
          ...
          // 2) Leaf text nodes for non-table elements
          const leafTexts = [];
          ...
          active.querySelectorAll('*').forEach(el => {
            ...
            // Skip if contained within a cell
            let inCell = false;
        ...
        for (const t of filtered) {
          ...
          const fs_ = Math.min(t.fontSize,32);
          const ch = color?`${color.r.toString(16).padStart(2,'0')}${color.g.toString(16).padStart(2,'0')}${color.b.toString(16).padStart(2,'0')}`:'120180';

          slide.addText(t.text, {
            x:xInch, y:yInch, w:wInch, h:hInch,
            fontSize:fs_, bold:t.bold, color:ch, fontFace:'Raleway',
            align:mapAlign(t.textAlign), margin:0, valign:'top',
