#!/usr/bin/env node
/**
 * PptxGenJS generator: screenshot skeleton + editable text boxes
 * Single run generates all 30 slides.
 */
...
const PPT_W = 13.333; // inches
const PPT_H = 7.5;
const CANVAS_W = 1280;
const CANVAS_H = 720;
const SX = PPT_W / CANVAS_W; // 0.010416
const SY = PPT_H / CANVAS_H;

// Slides config: [index, label, [ [variant_name, js_tab_func], ... ]]
const SLIDES = [
  [0, 'Portada', null],
  [1, 'Cap. 1', null],
  ...
  [7, 'D. Stock', null],
  [8, 'D. Masiva', null],
  [9, 'D. Satisfechos', null],
  [10, 'D. Microseguro', null],
  [11, 'D. Cancelaciones', null],
  [12, 'Asesores', [['Vanti', "asesoresTab('vanti')"], ['Xuma', "asesoresTab('xuma')"]]],
  [13, 'Iniciativas', null],
  [14, 'Evidencias', null],
  [15, 'Capacitaciones', null],
  [16, 'Monitoreo', null],
  [17, 'Cap. 2', null],
...
function isBold(el) {
  ...
  return fw === 'bold' || parseInt(fw) >= 600;
}
...
async function main() {
  ...
  for (const [num, label, tabs] of SLIDES) {
    try {
      ...
      for (const variant of variants) {
        ...
        // Deduplicate by region containment
        ...
        for (const t of textData) {
          ...
          for (const [ux, uy, uw, uh] of regions) {
            if (t.x >= ux && t.y >= uy && t.x + t.w <= ux + uw && t.y + t.h <= uy + uh) {
              contained = true;
              break;
            }
          }
        ...
        // ── Screenshot HYBRID (with text, icons visible) ──
        ...
        // Background: skeleton screenshot
        ...
        for (const t of filtered) {
          ...
          const xInch = t.x * SX;
          const yInch = t.y * SY;
          const wInch = Math.max(t.w * SX, 0.2);
          const hInch = Math.max(t.h * SY, 0.1);

          const color = parseColor(t.color);
          const fontSize = Math.min(t.fontSize * 0.75, 16);
  ...
  const outPath = path.join(DIR, 'INFORME_TELEVENTAS_PPTXGENJS.pptx');
