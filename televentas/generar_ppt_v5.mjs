#!/usr/bin/env node
/**
 * PptxGenJS v5: Skeleton + text overlays with cell-padding awareness
 * No native tables — everything is text overlay on skeleton screenshot
 */
...
function px2pt(px){return Math.round(px*72/96);} // CSS px -> PPT pt
...
function startServer(dir,port=0){
  return new Promise(r=>{
    const s=http.createServer((req,res)=>{
      ...
      fs.readFile(fp,(e,d)=>{if(e){res.writeHead(404);return res.end('Not found');}res.writeHead(200,{'Content-Type':types[path.extname(fp)]||'application/octet-stream'});res.end(d);});
    });s.listen(port,()=>r(s));
  });
}
...
async function main(){
  ...
  const port=server.address().port; console.log(`Server: http://localhost:${port}`);
  ...
  for(const [num,label,tabs] of SLIDES){
    try{
      ...
      for(const variant of variants){
        ...
        // ══ Extract all text with enhanced table cell handling ══
        const textData=await page.evaluate(()=>{
          ...
          function gs(el){return window.getComputedStyle(el);}

          // Icon positions
          const iconRects=[];
          active.querySelectorAll('svg,.lucide-ico,.ico').forEach(ico=>{
            const r=ico.getBoundingClientRect();iconRects.push({x:(r.left-sRect.left)/sx,y:(r.top-sRect.top)/sy,w:r.width/sx,h:r.height/sy});
          });
          ...
          const skipTags=new Set(['svg','path','rect','circle','line','polyline','polygon','use']);

          // Helper: extract a single text item from an element
          function addText(el,overrideText){
            if(skipTags.has(el.tagName)) return;
            if(el.closest('.lucide-ico')||el.classList.contains('lucide-ico')||el.classList.contains('ico')) return;
            const style=gs(el);
            if(style.display==='none'||style.visibility==='hidden') return;
            const text=overrideText!==undefined?overrideText:((()=>{let dt='';for(const c of el.childNodes){if(c.nodeType===3)dt+=c.textContent;}return dt.trim();})());
            ...
            // Extract CSS padding for cell elements
            ...
            results.push({
              ...
              padL,padT,padR,padB,
              isCell: el.tagName==='TD'||el.tagName==='TH',
            });
          }

          // 1) Extract <td> and <th> with ALL their text content
          active.querySelectorAll('td, th').forEach(cell=>{
            const fullText=cell.textContent.trim();
            if(!fullText||fullText.length<2) return;
            addText(cell, fullText);
          });

          // 2) Non-table, non-cell leaf text nodes
          active.querySelectorAll('*').forEach(el=>{
            if(skipTags.has(el.tagName)) return;
            if(el.tagName==='TD'||el.tagName==='TH') return; // already handled
            if(el.closest('td')||el.closest('th')) return; // inside cell, skip
            addText(el, undefined);
          });

          return results;
        });

        // Dedup by region (longest text first)
        ...
        for(const t of filtered){
          ...
          const xInch=t.x*SX, yInch=t.y*SY;
          const wInch=Math.max(t.w*SX*1.05,0.3), hInch=Math.max(t.h*SY*1.05,0.14);
          
          // Convert CSS padding to PPT margin (points)
          const margin=[px2pt(t.padT||0),px2pt(t.padR||0),px2pt(t.padB||0),px2pt(t.padL||0)];

          slide.addText(t.text,{
            x:xInch,y:yInch,w:wInch,h:hInch,
            fontSize:Math.min(t.fontSize*0.85,14),
  ...
  const outPath=path.join(DIR,'INFORME_TELEVENTAS_ESQUELETO_V5.pptx');
